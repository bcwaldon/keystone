...
def will_expire_soon(expiry, soon_seconds=30):
    """ Determines if expiration will occur 'soon'

    :param expiry: a datetime of the expected expiration
    :param soon_seconds: number of seconds that defines 'soon'
    :returns: boolean : true if expiration is within 30 seconds
    """
    soon = (datetime.datetime.utcnow() +
            datetime.timedelta(seconds=soon_seconds))
    return expiry < soon
...
class AuthProtocol(object):
    ...
    def _request_admin_token(self):
        ...
        try:
            ...
            raise ServiceError('invalid token expiry value')

        return (token, datetime_expiry)
